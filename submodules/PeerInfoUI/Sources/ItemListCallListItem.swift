...
public class ItemListCallListItem: ListViewItem, ItemListItem {
    ...
    public init(presentationData: ItemListPresentationData, dateTimeFormat: PresentationDateTimeFormat, messages: [Message], sectionId: ItemListSectionId, style: ItemListStyle, displayDecorations: Bool = true, date: Int32?) {
...
public class ItemListCallListItemNode: ListViewItemNode {
    ...
    public func asyncLayout() -> (_ item: ItemListCallListItem, _ params: ListViewItemLayoutParams, _ insets: ItemListNeighbors) -> (ListViewItemNodeLayout, () -> Void) {
        ...
        return { [weak self] item, params, neighbors in
            ...
            let headerDate: Int32 = item.date ?? item.messages.sorted(by: {$0.timestamp < $1.timestamp}).first!.timestamp
            let titleText = stringForDate(timestamp: headerDate, strings: item.presentationData.strings)
